...
impl Context {
    ...
    fn new(py: Python, width: u32, height: u32, format: PixelFormat) -> PyResult<Self> {
        let image =
            crate::image::Image::from_inner(raden::Image::new(width, height, format.into()));
    ...
    /// 内部の Image を返す。
    /// memoryview や numpy.asarray でピクセルデータに直接アクセスできる。
    fn image(&self, py: Python) -> Py<crate::image::Image> {
    ...
    fn clip_to_rect(&mut self, rect: &crate::geometry::Rect) {
        ...
        let (cx0, cy0, cx1, cy1) = match self.state.clip_rect {
            ...
            None => (
                0,
                0,
                self.state.image_width as i32,
                self.state.image_height as i32,
            ),
        };
    ...
    /// 合成モードを返す。
    fn comp_op(&self) -> crate::style::CompOp {
        self.state.comp_op.into()
    }

    /// 塗りつぶし規則を返す。
    fn fill_rule(&self) -> crate::style::FillRule {
        self.state.fill_rule.into()
    }

    /// 塗りつぶし色を返す。
    fn fill_color(&self) -> crate::style::Rgba32 {
        crate::style::Rgba32::from_inner(self.state.fill_color)
    }

    /// ストローク色を返す。
    fn stroke_color(&self) -> crate::style::Rgba32 {
        crate::style::Rgba32::from_inner(self.state.stroke_color)
    }

    /// 塗りつぶしグラデーションを返す。未設定の場合は None。
    fn fill_gradient(&self) -> Option<crate::gradient::Gradient> {
        self.state
            .fill_gradient
            .as_ref()
            .map(|g| crate::gradient::Gradient::from_inner(g.clone()))
    }

    /// 塗りつぶしパターンを返す。未設定の場合は None。
    fn fill_pattern(&self) -> Option<crate::pattern::Pattern> {
        self.state
            .fill_pattern
            .as_ref()
            .map(|p| crate::pattern::Pattern::from_inner(p.clone()))
    }

    /// ストロークグラデーションを返す。未設定の場合は None。
    fn stroke_gradient(&self) -> Option<crate::gradient::Gradient> {
        self.state
            .stroke_gradient
            .as_ref()
            .map(|g| crate::gradient::Gradient::from_inner(g.clone()))
    ...
    fn stroke_pattern(&self) -> Option<crate::pattern::Pattern> {
        self.state
            .stroke_pattern
            .as_ref()
            .map(|p| crate::pattern::Pattern::from_inner(p.clone()))
    ...
    /// ストロークのマイターリミットを返す。
    fn stroke_miter_limit(&self) -> f64 {
        self.state.stroke_miter_limit
    }

    /// ストロークの接続形状を返す。
    fn stroke_join(&self) -> crate::style::StrokeJoin {
        self.state.stroke_join.into()
    }

    /// ストロークの始点キャップを返す。
    fn stroke_start_cap(&self) -> crate::style::StrokeCap {
        self.state.stroke_start_cap.into()
    }

    /// ストロークの終点キャップを返す。
    fn stroke_end_cap(&self) -> crate::style::StrokeCap {
        self.state.stroke_end_cap.into()
    }

    /// ストロークのダッシュパターンを返す。
    fn stroke_dash_array(&self) -> Vec<f64> {
        self.state.stroke_dash_array.clone()
    }

    /// ストロークのダッシュオフセットを返す。
    fn stroke_dash_offset(&self) -> f64 {
        self.state.stroke_dash_offset
    }
    ...
    /// ユーザ行列を単位行列にリセットする。
    /// raden はメタ行列を別保持しないため、ユーザ行列のリセットのみ行う。
    fn user_to_meta(&mut self) {
        self.state.matrix.reset();
    }
