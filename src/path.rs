...
#[pyclass(eq, eq_int, name = "PathCmd")]
...
impl Path {
    ...
    /// 楕円弧を現在点から接続する。
    fn arc_to(&mut self, cx: f64, cy: f64, rx: f64, ry: f64, start: f64, sweep: f64, force_move_to: bool) {
        self.raden.arc_to(cx, cy, rx, ry, start, sweep, force_move_to);
    }
    ...
    fn add_path_transformed(&mut self, other: &Path, m: &crate::matrix::Matrix2D) {
        self.raden.add_path_transformed(&other.raden, &m.get_inner());
    }
